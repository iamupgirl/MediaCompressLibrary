#include <stdio.h>
#include "com_sherry_ffmpegdemo_FfmpegKit.h"
#include "ffmpeg.h"
 ...
 */
JNIEXPORT jint
JNICALL Java_com_sherry_ffmpegdemo_FfmpegKit_run
        (JNIEnv *env, jclass clazz, jobjectArray cmdline) {
    ...
    int cmdnum = 0;
    if (cmdline != NULL) {
        cmdnum = (*env)->GetArrayLength(env, cmdline);
...
JNIEXPORT jstring JNICALL Java_com_sherry_ffmpegdemo_FfmpegKit_configure(JNIEnv *env, jobject instance) {
    char info[10000] = {0};
